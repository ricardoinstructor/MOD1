...
class ResourcesManager {
    ...
    shareResource(title, url) {
        if (navigator.share) {
            navigator.share({
                title: title,
                text: `Recurso útil para MF0953_2: ${title}`,
                url: url
            });
        } else {
            // Fallback: copiar al portapapeles
            const text = `Recurso para MF0953_2 - ${title}: ${url}`;
...
function downloadGuide() {
    ...
    const guideContent = `
# Guía de Estudio MF0953_2 - Montaje de Equipos Microinformáticos

## Introducción
Esta guía está diseñada para ayudarte a preparar la certificación MF0953_2 en Montaje de Equipos Microinformáticos.
